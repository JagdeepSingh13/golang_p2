...
func ShortenURL(c *fiber.Ctx) error {
	...
	if err == redis.Nil {
		...
		if valInt <= 0 {
			// (time-to-live) remaining expiration time
	...
	// check input sent -> if an actual URL
	...
	if !helpers.RemoveDomainError(body.URL) {
		return c.Status(fiber.StatusServiceUnavailable).JSON(fiber.Map{"error": "can't access this domain"})
	}
	...
	// create a short url
	// either by the name sent by user or new uuid
	var id string
